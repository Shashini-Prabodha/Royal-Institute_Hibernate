...
public class QueryDAOImpl implements QueryDAO {
    ...
    @Override
    public List<Student> getCourseAllStudent() throws Exception {
        Session session = FactoryConfiguration.getInstance().getSession();
        Transaction transaction = session.beginTransaction();


        NativeQuery sqlQuery = session.createSQLQuery("select s.ID,s.name, s.address,s.contactNo,s.dob,s.gender  from student as s inner join registration r ,registration_course rc, course c where s.ID=r.student_ID and r.regNo=rc.registrations_regNo and rc.course_code=c.code  group by r.student_ID having count(*)=(select count(*) from course);");
        sqlQuery.setResultTransformer(Criteria.ALIAS_TO_ENTITY_MAP);
        List students = sqlQuery.list();
        List<Student> list=new ArrayList<>();
...
//    select s.name from student as s inner join registration r ,registration_course rc, course c where s.ID=r.student_ID and r.regNo=rc.registrations_regNo and rc.course_code=c.code  group by r.student_ID having count(*)=(select count(*) from course);
